...
import com.minook.zeppa.adapters.ContactFinderAdapter;
	...
	private void loadPossibleContacts() {
		...
		new AsyncTask<Object, Void, ProgressDialog>() {
			...
			protected ProgressDialog doInBackground(Object... params) {
				...
				ZeppaUserSingleton.getInstance().loadPossible(context);
			...
			protected void onPostExecute(ProgressDialog dialog) {
				...
				if (dialog.isShowing())
					dialog.dismiss();
				Log.d(TAG, "Found "
						+ ZeppaUserSingleton.getInstance().getHeldUsers()
								.size() + " possible connections");
